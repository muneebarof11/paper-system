import { class_id, section_id, subject_id, topic_id } from "../../params";
import * as Type from "./Types";
import {
    ADD_QUESTION_TYPE_RESPONSE,
    CLEAR_SELECTION,
    FETCH_ALL_QUESTION_TYPE_RESPONSE,
    REMOVE_QUESTION_TYPE_RESPONSE,
    TOGGLE_PAPER_SETTING,
    UPDATE_PAPER_SETTING,
...
export const fetchQuestionsWithTypes = (dispatch, is_section_id) => {
    ...
    if (is_section_id) query_string.section_id = section_id;
    ...
    return dispatch => {
        ...
        $api.getData(`get-question-types?${query_params}`)
            ...
            .then(res => {
                const query_params = $helper.buildQueryParams({
                    class_id,
                    subject_id,
                    section_id,
                    topic_id
                ...
                $api.getData(
                    `get-questions-by-class-subjects?${query_params}`
                ).then(res => {
                    const data = { data: res, message: "" };
...
export const confirmSelected = (dispatch, params, formData) => {
    ...
    formData.append(
        "confirmed_questions",
        JSON.stringify(selected_question[0])
    );
    ...
    return dispatch => {
        $api.postData("confirm-questions", formData)
            ...
            .catch(error => {
                dispatch(
                    confirmedQuestionResponse(selected_question, params.key, {})
                );
            });
    };
};
