...
let p_id = 0;
class UrduQuestions extends PureComponent {
    ...
    render() {
        return (
            <React.Fragment>
                <div className="container questions_container urdu_questions_container">
                        <nav>
                            <div className="nav nav-tabs nav-fill" id="nav-tabs" role="tablist">
                                {
                                    this.props.question_types.length > 0
                                        ? this.props.question_types.map((type, i) => {
                                            return (
                                                <a
                                                    className={`nav-item nav-link urdu-font ${i === 0 ? 'active' : ''}`}
                                                    id={`nav-${type.name}-tag`}
                                                    data-toggle="tab"
                                                    href={`#nav-${type.name}-${type.id}`}
                                                    role="tab"
                                                    aria-controls={`nav-${type.name}-${type.id}`}
                                                    aria-selected={i === 0}
                                                >
                                                    {type.title}
    ...
    checkIfToBeRendered(type) {
        if(this.props.section.rtl_name == type.title)
            p_id = type.id;
...
export default connect(mapStateToProps, mapDispatchToProps)(UrduQuestions);
