...
class IslGeneralQuestion extends Component {
    ...
    render() {
        ...
        let is_eng_none = q.en.statement.length <= 0 ? true : false;
        return (
            <div className="mt-4 mb-4 row pt-2 pb-2">
                <div className={`col-lg-6 ${is_eng_none ? 'd-none' : ''}`}>
                    <h5 className={`mb-2`}>
                ...
                <div className={`arabic-font text-right ${is_eng_none ? 'col-lg-11' : 'col-lg-5'}`}>
                    <h5 className="mb-2 text-right" style={{fontSize: '1.6rem'}}>
    ...
    createMarkup(html) {
        html = html.replace('<br>', '<span class="br"></span>');
        return {__html: `${html}`};
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(IslGeneralQuestion);
